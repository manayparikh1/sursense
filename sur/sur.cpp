...
Reading Detector::update(const float* mags, int bins, float bin_hz, float dt,
  ...
  float loudest = 0.0f;
  for (int b = lo; b <= hi; ++b) if (mags[b] > loudest) loudest = mags[b];
  ...
  // Re-decide about five times a second, not every frame.
  if (++frames_ >= 4 && voiced_ > 0.6f) { frames_ = 0; estimate(); }
  return current_;
}

void Detector::estimate() {
  float slow_max = 0.0f;
  for (int i = 0; i < kPitchClasses; ++i)
    if (slow_[i] > slow_max) slow_max = slow_[i];

  float score[kPitchClasses];
  for (int t = 0; t < kPitchClasses; ++t) {
    const float a = correlate(fast_, kMajor, t);
    const float b = correlate(fast_, kMinor, t);
    score[t] = (a > b ? a : b);
    if (slow_max > 0.0f) score[t] += kSustainBias * (slow_[t] / slow_max);
  }

  int best = 0;
  for (int t = 1; t < kPitchClasses; ++t) if (score[t] > score[best]) best = t;
  float rival = -2.0f;
  for (int t = 0; t < kPitchClasses; ++t)
    if (t != best && score[t] > rival) rival = score[t];

  float conf = (score[best] - rival) * 5.0f;
  if (conf > 1.0f) conf = 1.0f;
  if (conf < 0.0f) conf = 0.0f;
  conf *= voiced_ < 4.0f ? voiced_ / 4.0f : 1.0f;

  // Best-fitting scale: how much energy lands on each candidate's degrees.
  float rel[kPitchClasses], sum = 0.0f;
  for (int i = 0; i < kPitchClasses; ++i) {
    rel[i] = fast_[(best + i) % kPitchClasses];
    sum += rel[i];
  }
  const Scale* scale = nullptr;
  float bestfit = -1.0f;
  for (int s = 0; s < kScaleCount && sum > 0.0f; ++s) {
    float cover = 0.0f;
    for (int d = 0; d < 7; ++d) cover += rel[kScales[s].degrees[d]];
    if (cover / sum > bestfit) { bestfit = cover / sum; scale = &kScales[s]; }
  }

  // Easy to lock, hard to unlock: one bad frame must not discard a good answer,
  // but a real key change should eventually win.
  if (current_.locked) {
    if (best == current_.tonic) {
      challenger_count_ = 0;
      current_.scale = scale;
      if (conf > current_.confidence) current_.confidence = conf;
    } else {
      challenger_count_ = (best == challenger_) ? challenger_count_ + 1 : 1;
      challenger_ = best;
      if (challenger_count_ >= 14 && conf >= 0.45f) {
        current_ = {best, scale, conf, true};
        challenger_count_ = 0;
      }
    }
    return;
  ...
  if (history_count_ < 4) {
    history_[history_count_++] = best;
  } else {
    for (int i = 1; i < 4; ++i) history_[i - 1] = history_[i];
    history_[3] = best;
  }
  current_ = {best, scale, conf, false};

  bool steady = history_count_ == 4;
  for (int i = 1; steady && i < 4; ++i) if (history_[i] != history_[0]) steady = false;
  if (steady && conf >= 0.40f && voiced_ >= 2.2f) current_.locked = true;
}
